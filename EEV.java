...
public class EEV {
	public static double solve(Solution investmentEV){
		double result = 0;
		...
		//First stage decisions from EV problem
			for(int p:P){
				for(int t:T_p[p]){
					cplex.addEq(x_inv[p][t], investmentEV.opening[p][t]);
				}
			}
		...
		if(cplex.solve()){
			...
			System.out.println("Optimal objective value (EEV) is: "+result+".");
			System.out.println("Z1 (EEV) is: "+cplex.getValue(z1));
			System.out.println("Z2 (EEV) is: "+cplex.getValue(z2));
			System.out.println("Z3 (EEV) is: "+cplex.getValue(z3));
			System.out.println("Z4 (EEV) is: "+cplex.getValue(z4));
			for(int p:P){
				for(int t:T_p[p]){
